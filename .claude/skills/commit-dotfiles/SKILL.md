...
5. **Repeat** until `git status` shows no remaining tracked changes.
   Skip any files listed below under "Do not stage."
...
- `.claude/handoff.md` — leave untracked. These files are intentionally
  neither committed nor gitignored, so they surface here as a forcing
  function to revisit the decision.
  TODO: decide whether session handoff logs belong in git at all — they
  may be useful history, or they may be transient noise better left
