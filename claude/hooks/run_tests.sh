...
#
# The actual invocation is delegated to the quiet-tests runner so the canonical
# flags live in one place (~/.claude/scripts/quiet-tests.sh); its
...
# itself, so mirror its lookup rather than cd-ing here — this only has to decide
# whether the project has a suite at all. `:-` falls back to the current
