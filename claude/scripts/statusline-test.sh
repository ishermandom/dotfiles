...
#
# The script reads the wall clock to turn a reset timestamp into a burn pace
# and a countdown, so a stubbed `date` placed first on PATH freezes that
# clock. Each case gives its reset times as offsets from the frozen instant,
# so both the color and the countdown come out exact rather than approximate.
#
# Runs offline against a temporary tree, so it is safe to run anywhere.
# Requires jq, which statusline.sh uses to read its input.
...
mkdir "$test_root/bin"
# Quoted delimiter: $FROZEN_NOW must reach the stub unexpanded and resolve
# when the stub runs.
...
# A repository with one commit, then one line rewritten and one appended in
# the working tree.
...
# Builds the input object from the fields a case names, runs the script on
# that object, and leaves the printed line in `output`. A field left unnamed
# is absent from the JSON — the shape the script sees when a quota or a
# percentage is missing. Reset times are given as seconds from now and written
# out as absolute epochs, the form the real input carries.
run_statusline() { # run_statusline [--model M] [--context P] ...
  # A jq program of assignments applied left to right. The defaults come
  # first, so a field a case names overrides its default.
  ...
  # Built separately from the run, so a jq error surfaces on its own rather
  # than reaching the script as empty input.
  local input_object
...
# rate_limits is missing on API-key accounts, and a quota with no number has
# no segment to print.
...
# A window that has only just opened counts as 1% elapsed rather than 0%, so
# the first percent used still reads as on pace.
...
# With no reset time there is no window to pace against, so the quota falls
# back to fixed thresholds: green below 70%, yellow below 90%, red at or
# above 90%.
...
# The countdown shows the two coarsest units in play, and stops once the
# window it counts down to has arrived.
...
# Git state goes last, after the quotas, so the segments ahead of it hold
# their position as edits come and go.
...
# The fixture rewrote one line and appended another, and a rewrite counts as
